        ...
        $modules = @(
            ...
            if ($PSVersion -ge 7 -and $PSVersionTable.PSVersion.Minor -ge 2) {
                try {
                    ...
                    Write-Host "⚠️  Module load error: $(# PowerShell Version Compatibility Check
$PSVersion = $PSVersionTable.PSVersion.Major
$IsWindowsPowerShell = $PSVersionTable.PSEdition -eq 'Desktop'
$IsPowerShell7Plus = $PSVersion -ge 7
    ...
    if (Test-Path $psReadLineConfig) {
        ...
        if (Get-Module PSReadLine) {
            ...
            Set-PSReadLineKeyHandler -Key F9 -ScriptBlock {
                [Microsoft.PowerShell.PSConsoleReadLine]::Insert("Get-ComputerInfo | Select-Object -Property @{Name='Computer';Expression=`$_.CsName}, @{Name='OS';Expression=`$_.WindowsProductName}, @{Name='Version';Expression=`$_.WindowsVersion}, @{Name='RAM';Expression={'{0:N2} GB' -f (`$_.TotalPhysicalMemory/1GB)}}")
            }
