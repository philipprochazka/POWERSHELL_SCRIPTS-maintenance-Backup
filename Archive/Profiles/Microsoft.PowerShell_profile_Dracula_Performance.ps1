...
#region 🎨 Essential Theme Setup (Minimal - Modern v26+ Support)
...
if (-not $env:POSH_THEME) {
    if ($global:DraculaDebugEnabled) {
        Invoke-DraculaDebugStep -StepName 'ThemeSetup' -ScriptBlock {
            # Try modern Oh My Posh v26+ initialization
            ...
            if (Test-Path $modernInitScript) {
                try {
                    . $modernInitScript
                    $initialized = Initialize-ModernOhMyPosh -Mode "Performance"
                } catch {
                    Write-Verbose "Modern init failed, falling back: $($_.Exception.Message)"
                }
            }
        ...
        # Try modern Oh My Posh v26+ initialization (non-debug mode)
        $modernInitScript = Join-Path $PSScriptRoot "PowerShellModules\UnifiedPowerShellProfile\Scripts\Initialize-ModernOhMyPosh.ps1"
        $initialized = $false
        ...
        if (Test-Path $modernInitScript) {
            try {
                ...
                # Silent fallback in performance mode
            }
        }
